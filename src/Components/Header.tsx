...
import { socialMediaUrl } from "../static/Details";
import { colors, routePathNames } from "../static/constants";

/**
 * A Header component.
 * Used to show router links
 * @component
...
const Header : React.FC = (): JSX.Element => {
  ...
  return (
    ...
    <header className="fixed top-0 left-0 w-full bg-white shadow-md z-50 h-20 mx-auto md:flex justify-between py-2 max-width mb-200 dark:bg-black dark:text-white">
      <div className="flex justify-between items-center py-2 md:py-10">
        <div onClick={navigateToContent} className="cursor-pointer">
          <svg
      ...
      <nav className={` ${!isOpen ? "hidden" : null} text-center md:flex justify-between`}>
        <ul className="dark:text-light-content font-medium md:flex items-center md:space-x-5 md:mr-10">
          <li className="pb-1 md:pb-0"  >
            <NavLink to={HOME} onClick={navigateToContent} style={{color : pathname === HOME  && colors.headerLink }} >
              Home
            </NavLink>
          </li>
          <li className="pb-1 md:pb-0">
            <NavLink to={ABOUT} onClick={navigateToContent} style={{color : pathname === ABOUT && colors.headerLink}}>
              About
            </NavLink>
          </li>
          <li className="pb-1 md:pb-0">
            <NavLink to={TECHNOLOGIES} onClick={navigateToContent} style={{color : pathname === TECHNOLOGIES  && colors.headerLink }}>
              Technologies
            </NavLink>
          </li>
          <li className="pb-1 md:pb-0">
            <NavLink to={PROJECTS} onClick={navigateToContent} style={{color : pathname === PROJECTS  && colors.headerLink }}>
              Projects
            </NavLink>
          </li>
          <li className="pb-1 md:pb-0">
            <NavLink to={BLOGS} onClick={navigateToContent} style={{color : pathname === BLOGS  && colors.headerLink }}>
              My Blogs
            </NavLink>
          </li>
          <li>
            <NavLink to={CONTACT} onClick={navigateToContent} style={{color : pathname === CONTACT  && colors.headerLink }}>
              Contact
            </NavLink>
          </li>
        </ul>
